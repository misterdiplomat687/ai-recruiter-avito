...
async def send_candidate_card(dialog_id, conversation):
    card_text = "[B]New candidate from Avito[/B]\n\n"
    for msg in conversation:
        if msg["role"] == "user":
            card_text += f"Candidate: {msg['content']}\n"
        elif msg["role"] == "assistant":
            card_text += f"Bot: {msg['content']}\n"
    card_text += f"\nDialog ID: {dialog_id}"
...
async def install_get(request: Request):
    ...
    async with httpx.AsyncClient() as client_http:
        ...
        data = resp.json()
        logger.info(f"Install response: {data}")
        if "access_token" in data:
            tokens["access_token"] = data["access_token"]
            tokens["refresh_token"] = data["refresh_token"]
            # Register event handler for open lines messages
            await bitrix_call("event.bind", {
...
async def install_post(request: Request):
    ...
    logger.info(f"Install POST: {body}")
    code = body.get("code") or body.get("AUTH_ID")
    if body.get("AUTH_ID"):
        tokens["access_token"] = body["AUTH_ID"]
        tokens["refresh_token"] = body.get("REFRESH_ID", "")
...
async def webhook(request: Request):
    try:
        ...
        if body.get("event") == "ONIMBOTMESSAGEADD" or "data" in body:
            ...
            connector_mid = params.get("CONNECTOR_MID", "")
